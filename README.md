...
- **多地点标注**：在顶部「📌」标注框搜索并选择学校、医院、公司等地点，可同时保留多个自定义图钉；标注只调用地点联想，不会触发公交/驾车重算，并保存在本机浏览器；
- **地铁线路图层**：地图内嵌上海地铁线路走向、线路色和站点名称；首次打开默认勾选 **1、3、9、11、12 号线**，也支持「全选 / 全不选」及逐条勾选显示，站点标签会随线路筛选联动；只切换图层，不影响房源筛选和通勤计算；
...
- **📌 标注框**：输入地点并从联想结果中选择，即可在地图上增加一个独立图钉；重复操作可标注多个地点，不改变通勤目的地、不计算路线；点击图钉可删除单个标注，顶部按钮可清空全部标注；
- **🚇 地铁线路**：在蓝色栏中勾选要展示的线路，首次打开默认勾选 1、3、9、11、12 号线；支持全选/全不选，线路及站点数据已随 HTML 内嵌，运行时不请求额外的线路数据；
...
├── metro_lines.json       地铁线路/站点静态坐标与颜色(构建时内嵌进地图)
